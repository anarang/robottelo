...
class RHAI(Base):
    ...
    def register_client_to_rhai(self, activation_key, org):
        ...
        self.vm.run('wget -O /etc/yum.repos.d/rhel.repo {0}'.format(
            rhel6_repo))

        self.hostname = self.vm.run('hostname')
        print self.hostname
        if self.hostname is None:
            hostname = self.vm.run(
        ...
        if result.return_code != 0:
            self.vm.run('redhat-access-insights --test-connection')
            raise AccessInsightsError(
